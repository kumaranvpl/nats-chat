...
async def run(loop, process_id):
    ...
    async def enter_client_id_chat_handler(msg):
        ...
        if msg_client_id in tracking_client_ids:
            await msg.ack()
            print(f"Received a message on '{subject=} {reply=}': {d=} -> from {process_id=}")

            reply = {"id": "backend", "m": f"Helo {d['id']}!, received your msg=`{d['m']}` -> from {process_id=}"}
            await js.publish(f"backend.{process_id}", bytes(json.dumps(reply), "utf-8"))
        else:
            print(f"Unknown client id {msg_client_id}, going to nak it -> from {process_id=}")
            await msg.nak()
    ...
    async def enter_message_handler(msg):
        ...
        welcome = {"id": "backend", "m": f"Welcome to the chat! - {d['id']} -> from {process_id=}"}
        ...
        chat_id = d['id']
        await js.subscribe(f"chat.{chat_id}", cb=enter_client_id_chat_handler, ordered_consumer=False)

    # Subscribe to chatId
    await js.subscribe("enter.*", queue="enter_workers", cb=enter_message_handler, ordered_consumer=False)
